...
var us=st.gu.get(vid);if(!us){var _n=nw();st.uu.run(vid,'璁垮-'+vid.slice(0,6),'',_n,_n);us=st.gu.get(vid)};
function j(d,st){res.writeHead(st||200,{'Content-Type':'application/json'});res.end(JSON.stringify(d))}
if(p==='/api/health'&&m==='GET')return j({ok:true})
if(p==='/api/identity/manual'&&m==='POST'){rl('i'+vid,30,60000);var b=await pb(req);var nm=(b.displayName||'').trim().slice(0,40),av=(b.avatarUrl||'').trim().slice(0,300);if(!nm)throw eh(400,'鏄电О涓嶈兘涓虹┖');if(av&&!av.startsWith('https://'))throw eh(400,'av');var _t=nw();st.uu.run(vid,nm,av,_t,_t);us=st.gu.get(vid);return j({ok:true,displayName:us.display_name})}
...
if(eM){if(m==='GET'){var exps=st.ge.all(ty,iid).map(function(r){return{id:r.id,body:r.body,createdAt:r.created_at,displayName:r.display_name||'鍖垮悕',avatarUrl:r.avatar_url||''}});return j({experiences:exps})}
if(m==='POST'){rl('e'+vid,12,60000);ru(us);var b=await pb(req),tx=(b.body||'').trim();if(!tx)throw eh(400,'鍐呭涓嶈兘涓虹┖');if(tx.length>2000)throw eh(400,'鍐呭杩囬暱');st.ae.run(crypto.randomUUID(),ty,iid,vid,tx,nw());return j({ok:true})}}
