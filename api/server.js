...
function cr(t,o,a,isql){var n=q;return{li:n('SELECT t.*,u.display_name an FROM '+t+' t LEFT JOIN users u ON u.id=t.'+a+' ORDER BY t.'+o+' DESC LIMIT ? OFFSET ?'),ge:n('SELECT t.*,u.display_name an FROM '+t+' t LEFT JOIN users u ON u.id=t.'+a+' WHERE t.id=?'),cn:n('SELECT COUNT(*)n FROM '+t),de:n('DELETE FROM '+t+' WHERE id=?'),in_:isql?n(isql):null}};
var T={};
...
if(m==='POST'||m==='PUT'){var BODY=await pb(req).catch(function(e){throw e})}else{var BODY=null}
...
if(typ==='tools'){T.tools.in_.run(id,ti,(BODY.description||'').trim().slice(0,500),(BODY.url||'').trim().slice(0,500),vid,((BODY.tags||[]).join?BODY.tags.join(','):String(BODY.tags||'')).trim().slice(0,200),_n,0,_n)}
if(typ==='discussions'){T.disc.in_.run(id,ti,(BODY.body||'').trim().slice(0,8000),vid,(BODY.category||'general').trim().slice(0,50),_n,0,_n)}
if(typ==='submissions'){T.sub.in_.run(id,ti,(BODY.body||'').trim().slice(0,8000),vid,((BODY.tags||[]).join?BODY.tags.join(','):String(BODY.tags||'')).trim().slice(0,200),'visible',_n,_n)}
